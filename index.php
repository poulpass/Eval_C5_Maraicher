            ...
            <div class="tab-pane active table-responsive" id="home">
               <table class="table">
                  ...
                  <tbody>
                     <?php 
                     //liste complète
                        $dbconn = pg_connect("host=localhost port=5432 dbname=bdpoireau_ok user=admin password=admin");
...
ORDER BY pro_leg,pro_nom");
                        ...
                        while($donnees = pg_fetch_assoc($result)){
                           if ($donnees['sum'] < 5) {
                           $affichage .= '<tr>
                              <td class="warning">⚠ '.$donnees['pro_nom'].'</td>
                              <td class="warning">'.$donnees['sum'].'</td>
                           </tr>';
                           }
                           else {
                           $affichage .= '<tr>
                              <td>'.$donnees['pro_nom'].'</td>
                              <td>'.$donnees['sum'].'</td>
                           </tr>';
                        }
                        }
               ...
               <table class="table">
                  ...
                  <tbody>
                     ...
                     //liste Fruits
                        ...
                        $result = pg_query($dbconn, "SELECT pro_nom, sum(st) 
FROM (SELECT pro_leg,pro_nom, -sto_qte as st
FROM stock 
INNER JOIN produit ON pro_id=spro_id
WHERE sto_pert = True 
...
SELECT pro_leg, pro_nom, -con_qte as st 
FROM contenu 
INNER JOIN produit ON cpro_id = pro_id) as s WHERE pro_leg = false
GROUP BY pro_leg,pro_nom
ORDER BY pro_leg,pro_nom ");
...
SELECT pro_leg, pro_nom, -con_qte as st
FROM contenu
INNER JOIN produit ON cpro_id = pro_id) 
as s WHERE pro_leg = True
GROUP BY pro_leg,pro_nom
ORDER BY pro_leg,pro_nom");
         ...
         <div class="row">
            ...
            <div class="col-md-5 right-panel">
                  ...
                  <div class="container">
                     <div class="row">
                        ...
                        <div class="col-md-4">
                           <div class="form-group">
                              <label for="quantityToAdd">Quantité</label>
                              <input type="number" class="form-control" name="formAjouter" id="" placeholder="">
                           </div>
                        </div>
                        <div class="col-md-8">
                           <div class="form-group">
                              <label for="itemToAdd">Fruit/Légume</label>
                              <!-- Menu déroulant -->
                              <select class="form-control" name="formAjouter2" id="">
               ...
               <?php 
                     ...
                     $nombre = $_POST['formAjouter'];
                     $produit = $_POST['formAjouter2'];


                     $result = pg_query($dbconn, "UPDATE stock SET sto_qte = sto_qte+'".$nombre."' WHERE spro_id = '".$produit."' ");
                     echo $nombre;
                     echo $produit;
                     echo "Je comprends pas pourquoi ça change dans la BDD et pas sur la page !";
